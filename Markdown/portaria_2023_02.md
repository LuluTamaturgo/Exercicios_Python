# 🗂️ Descrição da Tabela: portaria_2023_02
A tabela portaria_2023_02 armazena os valores estimados de repasse por modalidade de ensino, os coeficientes de ponderação VAAT/VAAR e a estimativa de receita anual por município para o exercício de 2023, segundo as diretrizes da Portaria Interministerial nº 2/2023. Os dados estão organizados por tipo de atendimento escolar, diferenciando entre complementações RP (VAAT) e RC (VAAR), e contemplam modalidades específicas como educação especial, EJA, indígena, quilombola e profissionalizante.
...
| `matriculas_totais`                                        | decimal(12,2)               | Total de matrículas consideradas no cálculo dos repasses.                  |
| `coeficiente_vaaf`                                         | decimal(20,12)              | Coeficiente de ponderação VAAT/VAAR (VAAF) para distribuição dos recursos. |
| `estimativa_receita_vaaf`                                  | decimal(18,2)               | Estimativa de receita total via complementação da União (VAAT/VAAR).       |
| `municipality_id`                                          | bigint, FK                  | Identificador do município associado (chave estrangeira).                  |
