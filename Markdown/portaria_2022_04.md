🗂️ Descrição Geral da Tabela portaria_2022_04
A tabela portaria_2022_04 registra os valores de repasse por aluno (em R$) referentes ao ano de 2022, conforme definidos por uma quarta portaria normativa do Fundeb. Esses valores são utilizados como base para:

Cálculo do Valor Aluno Ano Fundeb (VAAF);

Estimativa de receita para cada município;

Garantia de equidade na distribuição de recursos, considerando as realidades regionais e educacionais.

A tabela contempla diversas etapas e modalidades de ensino, a localização da escola (urbana ou rural), o regime de ensino (parcial ou integral) e as formas de complementação de recursos do Fundeb, como:
...
| ---------------------------------------------------------- | -------------- | ----------- | --------------------------------------------------------------------- |
| `id`                                                       | bigint         | Sim (PK)    | Identificador técnico exclusivo da linha da tabela.                   |
| `creche_integral_rp`                                       | decimal(8,2)   | Não         | Valor por aluno na creche integral – repasse próprio (RP).            |
| `creche_parcial_rp`                                        | decimal(8,2)   | Não         | Valor por aluno na creche parcial – repasse próprio (RP).             |
...
| `eja_avaliacao_processo_rp`                                | decimal(8,2)   | Não         | Educação de Jovens e Adultos com avaliação por processo – RP.         |
...
| `eja_avaliacao_processo_rc_formacao_alternancia`           | decimal(8,2)   | Não         | EJA com processo avaliativo e alternância – RC.                       |
| `eja_integrada_profissional_rc_formacao_alternancia`       | decimal(8,2)   | Não         | EJA integrada à formação profissional com alternância – RC.           |
...
| `matriculas_totais`                                        | decimal(12,2)  | Não         | Total de matrículas consideradas para o cálculo do Fundeb.            |
...
| `municipality_id`                                          | bigint         | Sim (FK)    | Chave estrangeira que referencia o município relacionado.             |
